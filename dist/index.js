...
app.get("/hero-physics.js", (c) => {
    const content = readFileSync(join(__dirname, "lib", "hero-physics.ts"), "utf-8");
...
function getHeroCircles() {
    ...
    [stroke1, stroke2].forEach((drawable) => {
        generator.toPaths(drawable).forEach((path) => {
            paths += `<path d="${path.d}" fill="none" stroke="${path.stroke}" stroke-width="${path.strokeWidth}" />`;
        });
    });
    generator.toPaths(arrowLine).forEach((path) => {
        paths += `<path d="${path.d}" fill="none" stroke="${path.stroke}" stroke-width="${path.strokeWidth}" />`;
    });
...
function parseDiagram(instance) {
...
app.get("/svg/*", (c) => {
    ...
    const parsedDiagrams = instances.map((instance, idx) => {
        ...
        if (!parsed)
            return null;
        return roughVennDiagram(parsed.titles, parsed.intersections, idx, parsed.universalSet);
    });
...
app.get("/img-svg/*", (c) => {
    ...
    const instances = segments.slice(1);
    if (instances.length === 0) {
        return c.text("img-svg api - add diagram: /img-svg/Title-1.Title-2.Title-3");
    }
    const parsedDiagrams = instances.map((instance, idx) => {
        const parsed = parseDiagram(instance);
        if (!parsed)
